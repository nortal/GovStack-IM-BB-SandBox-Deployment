...
log "Waiting for database ${XROAD_DB_HOST}..."
wait_for_db() {
  local count=0
  local max=$1
  ...
  if ((count>=$max)); then 
    warn "Unable to determine database status in $count retries"
  else
    log "Database available, continuing..."
  fi
}
wait_for_db 60
...
RECONFIG=(xroad-signer xroad-center)
...
if [[ -n "$XROAD_ADMIN_USER" ]] && ! getent passwd "$XROAD_ADMIN_USER" &>/dev/null; then
  ...
  echo "xroad-center xroad-common/username string ${XROAD_ADMIN_USER}" | debconf-set-selections
fi
XROAD_ADMIN_USER=
...
if [ ! -f /etc/xroad/ssl/nginx.crt ]; then
  log "Generating new SSL key and certificate for the admin UI"
  "$XROAD_SCRIPT_LOCATION/generate_certificate.sh" -n nginx -f -S -p 2>&1 >/dev/null | sed 's/^/   /'
fi
...
if [ ! -f ${DB_PROPERTIES} ]; then
  ...
  cat >/etc/xroad/db.properties <<EOF
adapter = postgresql
encoding = utf8
username = centerui
...
EOF
  ...
  echo "xroad-common xroad-common/database-host string ${XROAD_DB_HOST}:${XROAD_DB_PORT}" | debconf-set-selections
fi
...
if [[ -f /var/lib/xroad/conf_backup.tar && ! -f /etc/xroad/.initialized ]]; then
  ...
  tar -C / -xf /var/lib/xroad/conf_backup.tar etc/xroad/signer var/lib/xroad/dbdump.dat
  chown -R xroad:xroad /etc/xroad/signer /var/lib/xroad/dbdump.dat
...
if [ ! -d /home/ca/CA ]; then
  # perhaps a mounted volume, restore packaged files
  log "Populating TEST-CA volume"
  cp -a $PACKAGED_CONFIG/ca /home
  chown -R ca:ca /home/ca
fi

if [[ ! -f /home/ca/CA/.init  && -f /var/lib/xroad/ca_backup.tar ]]; then
  log "Restoring TEST-CA"
  tar -C /home/ca -xf /var/lib/xroad/ca_backup.tar 2>&1 | sed 's/^/    /'
  chown -R ca:ca /home/ca
fi
